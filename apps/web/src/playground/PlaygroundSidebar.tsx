...
export function PlaygroundSidebar({
  ...
  return (
    <nav
      className="glass-panel flex flex-col gap-2 overflow-x-auto rounded-2xl border border-border p-1.5 lg:overflow-visible"
      aria-label={t('label')}
    >
      {projectName ? (
        <p className="hidden px-2 pt-1 text-xs font-semibold text-text-primary lg:block">
          {projectName}
        </p>
      ) : null}
      ...
      {PLAYGROUND_VIEWS.map((view) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex min-w-[9rem] items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition-colors lg:min-w-0 lg:w-full',
              isActive
